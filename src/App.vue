<template>
  ...
  <div class="help" v-show="display_help" :class="{ 'help-on': display_help }">
    <div class="help-content">
      ...
      <ul>
        <li><kbd>Ctrl</kbd>+<kbd>1</kbd>&nbsp;翻译搜索框内容</li>
        <li><kbd>Ctrl</kbd>+<kbd>2</kbd>&nbsp;必应搜索</li>
        <li><kbd>Ctrl</kbd>+<kbd>3</kbd>&nbsp;谷歌搜索</li>
        <li><kbd>Ctrl</kbd>+<kbd>4</kbd>&nbsp;开发者搜索</li>
      </ul>
    </div>
  </div>
  <!-- 搜索栏 -->
  ...
  created() {
    ...
    document.onkeydown = e => {
      ...
      if (e.ctrlKey && e.key == '1') {
        window.location.href = `https://fanyi.baidu.com/#en/zh/${this.keywords}`
      }
      // Shift + W: 必应搜索
      if (e.ctrlKey && e.key == '2') {
        window.location.href = `https://cn.bing.com/search?q=${this.keywords}`
      }
      // Shift + E: 谷歌搜索
      if (e.ctrlKey && e.key == '3') {
        window.location.href = `https://www.google.com/search?q=${this.keywords}`
      }
      // Shift + E: 百度开发者搜索
      if (e.ctrlKey && e.key == '4') {
        window.location.href = `https://kaifa.baidu.com/searchPage?wd=${this.keywords}&module=SEARCH`
      }
    }
  },
  mounted() {
  ...
  watch: {
    ...
    keywords() {
      ...
      Axios.get('https://www.baidu.com/sugrec', {
        ...
        if (!Object.keys(res.data).length) return  // 搜索框为空时不请求
        let tmp = []
        ...
        if (tmp.length) this.result_list = tmp
      })
      .catch(err => {
        console.log(err)
      })
...
.help-on
  ...
  .help-content
    animation HeloOnAnimate-Content .3s cubic-bezier(.5, 0, .5, 1.5) forwards
...
@keyframes HelpOnAnimate-Mask
  ...
  100%
    opacity 1
@keyframes HeloOnAnimate-Content
  0%
    transform scale(.8)
  100%
    transform scale(1)
