<template>
  <div id="vSettings" class="d-flex blur">
    <!-- 设置面板 -->
    <div class="panel p border-radius-lg">
      ...
      <div
        ...
        @click="data.showWeatherSettings = true"
        style="height: 2rem"
      >
        显示天气组件设置
      </div>
    ...
    <!-- 二级面板 - 天气 -->
    <div class="weather position-absolute p m-auto w-100 border-radius-lg" v-show="data.showWeatherSettings">
      ...
      <div
        ...
        @click="data.showWeatherSettings = false"
        style="height: 2rem"
      >返回</div>
    </div>
  </div>
      ...
      useCustomizeWallpaper: () => {
        ...
        reader.onloadend = () => {
          localforage.setItem('01', reader.result, err => {
            ...
            alert('设置成功（刷新生效）')
            console.log(store.state.wallpaper)
          })
        }
      },
      setLocation: id => {
        ...
        alert('设置成功（刷新生效）')
        data.showWeatherSettings = false
      }
    }
